...
import com.fintech.ledger.constants.LedgerConstants;
import com.fintech.ledger.model.AuthenticatedUser;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        final String authHeader = request.getHeader(LedgerConstants.AUTH_HEADER);
        if (authHeader == null || !authHeader.startsWith(LedgerConstants.BEARER_PREFIX)) {
            filterChain.doFilter(request, response);
            return;
        }

        final String jwt = authHeader.substring(LedgerConstants.BEARER_PREFIX.length());
        ...
        try {
            if (jwtService.isTokenValid(jwt)) {
                ...
                log.debug("JWT AUTH OK | userId={} | email={} | uri={}", userId, email, request.getRequestURI());
            } else {
